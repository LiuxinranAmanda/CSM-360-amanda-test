...
import {
  getCustomer, customerRiskHistory, customerSnapshots, getDossier, customerLogs, listCadenceRules,
  customerJiraTickets, customerMonthly,
} from "@/lib/queries";
...
export default async function CustomerDetailPage({ params }) {
  ...
  const isAdmin = canSeeAll(session.role); // admin + viewer 都能打开任意客户

  const customer = await getCustomer(params.id, session.tpids?.length ? session.tpids : session.pid, isAdmin);
  if (!customer) notFound();
  await logVisit(session, "/customer"); // 页面类别,不带具体客户id(使用度统计,非行为跟踪)

  const [risks, snapshots, dossier, logs, cadenceRules, jiraTickets, monthly] = await Promise.all([
    customerRiskHistory(customer.id),
    customerSnapshots(customer.id, 14),
    getDossier(customer.id),
    customerLogs(customer.id, 30),
    listCadenceRules(),
    customerJiraTickets(customer.id),
    customerMonthly(customer.id),
  ]);

  const aiEval = nonEmpty(customer.ai_evaluation);
  const nextAction = nonEmpty(customer.next_action);
  const coveragePct = fmtRatioPct(customer.service_coverage);
  ...
  const shopCount = Array.isArray(customer.profile_ids) ? customer.profile_ids.length : null;
    ...
    <>
      ...
      <main className="page cust-two-col">
        <aside className="cust-side">
          <AskAi customerId={customer.id} customerName={customer.name}
                 inviteScript={nextAction?.script || null}
                 dossierConclusion={dossierConclusion} />
        </aside>
        <div className="cust-main">
        <Link href="/" className="backlink">← 返回{isAdmin ? "全部客户" : "我的客户"}</Link>
              ...
              {(customer.top_asins[0]?.list
                ...
                <div key={site.cc || si} style={{ marginBottom: si < customer.top_asins.length - 1 ? 14 : 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    {(site.list || []).map((t, i) => (
                      <a key={t.a + si} href={amazonUrl(site.cc || t.cc, t.a)} target="_blank" rel="noreferrer"
                        className="asinchip">
        ...
        <div className="card">
          ...
          <div className="kyc-contract-info">
            ...
            {customer.contract_end && (
              <span className={"tag " + (daysUntil(customer.contract_end) <= 60 ? "seg-risk" : "t-low")}>剩余 {daysUntil(customer.contract_end)} 天</span>
            )}
            {customer.contract_end && daysUntil(customer.contract_end) <= 60 && (
